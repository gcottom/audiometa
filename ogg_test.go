...
func TestOggCRCTable(t *testing.T) {
	...
	result := oggCRCTable(poly)
	assert.Equal(t, expected1, result, "oggCRCTable(%d) should return %v", poly, expected1)

}

func TestOggCRCUpdate(t *testing.T) {
	// Test cases for updating CRC with different inputs
	testCases := []struct {
		name     string
		crc      uint32
		tab      *crc32Table
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result := oggCRCUpdate(tc.crc, tc.tab, tc.input)
			if result != tc.expected {
				t.Errorf("Expected CRC: %08X, got: %08X", tc.expected, result)
			}
		})
	}
}
